...
def buildMoleculeDDI(ddi_A_final, average_projection):
    row_sums = torch.sum(average_projection, dim=1, keepdim=True)  
    normalized_matrix = average_projection / row_sums
    normalized_matrix = normalized_matrix.to(torch.float32)
    ddi_A_final = ddi_A_final.to(torch.float32)
    temp = torch.mm(normalized_matrix.t(), ddi_A_final)
    molecule_ddi = torch.mm(temp, normalized_matrix)
    # molecule_ddi = 1 - molecule_ddi
    return molecule_ddi
